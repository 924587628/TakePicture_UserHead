...
public class YCLTools {
	...
	private static String YCL_FOLDER_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator
			+ "ycl/";
	private String TEMP_PIC_NAME_JPG ="_temp_headImg.jpg";
	...
	public void upLoadImage(int requestCode, int resultCode, Intent data) {
		if (resultCode == Activity.RESULT_OK) {
			...
			case YCLTools.MODE_UPLOAD_IMAGE_ABLUME:
				...
				if (cursor == null) {
					onActivityResult(selectedImage.getPath());
					return;
				}
				cursor.moveToFirst();
	...
	private void onActivityResult(String path) {
		Intent intent = new Intent(activity, CropImageViewActivity.class);
